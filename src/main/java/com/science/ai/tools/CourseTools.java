...
import com.science.ai.service.ICourseReservationService;
import com.science.ai.service.ICourseService;
...
public class CourseTools {
    ...
    public List<Course> queryCourse(@ToolParam(required = false, description = "查询的条件") CourseQuery query) {
        if (query == null) {
            return Collections.emptyList();
        }

        QueryChainWrapper<Course> wapper = courseService.query()
                .eq(query.getType() != null, "type", query.getType())
                .le(query.getEdu() != null, "edu", query.getEdu());

        if (query.getSorts() != null && !query.getSorts().isEmpty()) {
            for (CourseQuery.Sort sort : query.getSorts()) {
                wapper.orderBy(true, sort.getAsc(), sort.getField());
            }
        }
        return wapper.list();
